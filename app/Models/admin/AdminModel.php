...
class AdminModel extends Model
{
    ...
    public function get_all_themes(){
        return $this->themes->orderBy('kode_theme', 'ASC')->get();
    }
    public function get_all_themes_video(){
        return $this->themes_video->get();
    }
    ...
    {
        $builder = $this->pembayaran;
        $builder->select('order.*,users.*,pembayaran.*,data.*,order.created_at as tgl_daftar, pembayaran.created_at as tgl_bayar, pembayaran.status as statusPembayaran,order.status as statusWeb');
        $builder->join('users', 'users.id = pembayaran.id_user', 'left');
        $builder->join('order', 'order.id_user = pembayaran.id_user', 'left');
        $builder->join('data', 'data.id_user = pembayaran.id_user', 'left');
