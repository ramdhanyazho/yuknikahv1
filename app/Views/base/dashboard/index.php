        ...
        <div class="col-xl-12 col-lg-12 mb-4">
            <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    <h6 class="m-0 font-weight-bold " style="color:red"><a href="<?= base_url('user/invoice') ?>" >Invoice</a> : Selesaikan pembayaran anda sebelum <?= $tglExpFormated ?> || Status Undangan : <?php if($today < $tglExp) { echo "Trial"; }else{ echo "Tidak Aktif";}?></h6>
                    
                    <?php }else if($pembayaran[0]->status == 1){ ?>
                    <h6 class="m-0 font-weight-bold " style="color:orange"><a href="<?= base_url('user/invoice') ?>" >Invoice</a> : Pembayaran anda menunggu dikonfirmasi || Status Undangan : <?php if($today < $tglExp) { echo "Trial"; }else{ echo "Tidak Aktif";}?></h6>
                    
                    <?php } else if($pembayaran[0]->status == 2 && $today >= $tglNonaktif ){ 
                    ?>
                    <h6 class="m-0 font-weight-bold " style="color:red"><a href="<?= base_url('user/invoice') ?>" >Invoice</a> : Masa Aktif Undangan sudah habis pada tanggal <?= $tglNonaktifFormated ?>, Silahkan lakukan pembayaran lagi untuk memperpanjang masa aktif undangan </h6>
                    <?php }else{ ?>
            ...
            <div class="card">
                ...
                <div>
                    ...
                    <div class="card-footer text-center">
                        <a class="m-0 text-primary card-link" href="<?= base_url('user/ucapan') ?>">Lihat Lebih <i class="fas fa-chevron-right"></i></a>
                    </div>
                </div>
            </div>
        </div>
